...
export default defineConfig(({ mode }) => {
  // 把 .env.local / .env 里的非 VITE_ 变量(豆包 key 等)载入 process.env,
  // 让 vite node 侧 (admin-ai 插件 + ark 代理) 直接读到, 无需手动 export。
  const env = loadEnv(mode, process.cwd(), '');
  for (const k of ['ARK_API_KEY', 'ARK_TEXT_MODEL', 'ARK_IMAGE_MODEL', 'ARK_BASE_URL']) {
