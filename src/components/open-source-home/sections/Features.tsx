...
const FEATURES: FeatureEntry[] = [
  {
    ...
    title: 'AI 会分清：\n哪些已经确认，哪些只是线索，\n哪些必须等你来定。',
    body: [
      '普通 AI 最大的风险，不是答不出来，而是把不确定的内容说得很肯定。',
      '益语智库会把信息按可信程度放好：可以先记下线索，但不会把未经确认的内容当成权威依据；遇到拿不准的地方，它会明确提出来，让你确认之后再进入正式判断。',
    ],
    takeIcon: Shield,
    take: '让 AI 更可信，而不是更会编。',
  ...
  },
  {
    ...
    title: '每一次提问和生成，\n都能接上这个项目的历史。',
    body: [
      '每个客户或项目都有独立工作台。聊天、文件、战略、合同、提案、复盘和待澄清事项，都围绕同一个对象沉淀。',
      '你可以让 AI 生成整段内容，也可以只改一句话、一个词，或翻译一个句子。',
      '它不是从空白开始，而是在这个项目的历史里帮你调整表达。',
    ],
    takeIcon: User,
    take: '让背景不再反复丢失。',
    href: FUTURE_ROUTES.features,
  ...
  },
  {
    ...
    title: '像一位资深战略顾问，\n陪你看清每个重要项目。',
    body: [
      '益语智库不只是帮你写材料，而是持续陪你理解每一个重要项目：它在做什么，真正价值是什么，正在和谁合作，当前卡在哪里，下一步重点要推进什么。',
      '它让项目不只是散落的会议、文件和报告，而是一条可以持续理解、持续更新的合作脉络。',
    ],
    takeIcon: Heart,
    take: '让项目陪伴不再断线。',
    href: FUTURE_ROUTES.features,
  },
  {
    ...
    title: '多年项目不再散落，\n所有动作和文件都能串成一条线。',
    body: [
      '一个项目做三年、五年、十年，最怕材料散在人、群、网盘和旧文档里。',
      '益语智库会把合同、会议、提案、任务、沟通和关键文件按时间线串起来。写合同、提案、汇报时，系统可以回到项目历史里找背景；文件也能随时查看和下载，不再只靠人脑记住。',
    ],
    takeIcon: Download,
    take: '让长期项目有迹可循。',
    href: FUTURE_ROUTES.features,
  },
  {
    ...
    title: '把战略拆到能执行，\n把任务分给人和 AI 同事。',
    body: [
      '行动者经常被日常事务埋住，只知道很忙，却很难抬起头看清方向。',
      '战略拆解不是写一份宏大的规划，而是把战略变成目标，把目标变成任务，再分配给同事和机器人同事一起推进。',
      '脚踏实地重要，抬头看路更重要。',
    ],
    takeIcon: Target,
    take: '让方向变成今天能推进的行动。',
  ...
  },
  {
    ...
    title: '给 AI 一个办公室，\n它才像同事一样把事做好。',
    body: [
      '你可以一次性把一天要做的事情交代给机器人同事。它不是在聊天框里凭空猜答案，而是在益语智库这个工作环境里，沿着项目线索、文件、任务和历史背景去推进工作。',
      '益语智库给 AI 同事准备了资料、任务、权限和过程记录，让它能真正进入组织协作。',
    ],
    takeIcon: Users,
    take: '让 AI 从工具变成可协作的同事。',
    href: FUTURE_ROUTES.features,
  },
];
      ...
      <Container>
        ...
        <Reveal>
          <div className="text-center max-w-3xl mx-auto">
            <div className="flex items-center justify-center gap-3 mb-6">
              ...
              <span className="text-[12px] font-semibold tracking-[0.18em] text-os-indigo">功能模块</span>
              <span className="h-px w-7 bg-os-indigo/50" />
            </div>
            ...
            <p className="mt-6 text-[15px] sm:text-[16px] leading-[1.85] text-os-muted">
              益语智库不是把 AI 功能堆在一起，而是让 AI 真正进入组织工作的关键环节：知道什么可信，记得项目历史，看清合作脉络，串起长期时间线，拆出可执行任务，并以同事身份参与推进。
            </p>
          </div>
        </Reveal>
        ...
        */}
          ...
          {FEATURES.map((f, i) => {
            ...
            return (
              <Reveal key={f.num} delay={(i % 3) * 90 + Math.floor(i / 3) * 120} className="h-full">
                <a
                  href={f.href}
                  aria-label={`了解能力 ${f.num}：${f.title.replace(/\n/g, ' ')}`}
                  onClick={
                    f.lightbox
                      ? (e) => {
                          e.preventDefault();
                  ...
                  <div className="relative flex flex-col h-full">
                    <div className="grid grid-cols-[auto_1fr] gap-x-5 gap-y-5 items-start">
                      ...
                      <div className="space-y-2.5">
                        {f.body.map((para, idx) => (
                          <p key={idx} className="text-[13.5px] leading-[1.9] text-os-muted">{para}</p>
                        ))}
                      </div>
                    </div>
        ...
        <div className="mt-14 text-center">
          ...
          <p className="mt-4 text-[14px] text-os-muted">了解益语智库如何帮你落地这些能力</p>
        </div>
      </Container>
