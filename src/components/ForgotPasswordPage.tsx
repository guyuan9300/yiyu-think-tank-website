...
export function ForgotPasswordPage({ onNavigate }: ForgotPasswordPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const result = await sendVerifyCode('email', normalizedEmail, 'reset');
      if (!result.ok) {
        setError(result.error || '发送失败，请稍后重试');
      } else {
        setSuccess('验证码已发送，请查收邮箱后继续重置密码。');
        setTimeout(() => {
          const encoded = encodeURIComponent(normalizedEmail);
          window.location.href = `${window.location.pathname}?page=reset-password&channel=email&target=${encoded}`;
        }, 800);
      }
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="bg-white/80 backdrop-blur rounded-3xl shadow-xl border border-border/40 p-8">
          ...
          <p className="mt-2 text-sm text-muted-foreground">输入注册邮箱，我们会发送验证码用于重置密码。</p>
