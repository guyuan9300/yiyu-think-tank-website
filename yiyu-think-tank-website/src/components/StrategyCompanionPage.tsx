      ...
      <main className="max-w-6xl mx-auto px-8 py-12 pt-24">
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-8 md:p-10 mb-6">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-white text-xl font-semibold">年度战略终点</h2>
                  <p className="text-slate-400 text-sm">North Star & Key Deliverables</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-5 border border-white/10">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-amber-400 text-xs font-semibold uppercase tracking-wider">North Star</span>
                  </div>
                  <p className="text-white text-lg font-medium leading-relaxed">
                    {selectedClient?.northStarMetric || '（请在后台填写北极星指标）'}
                  </p>
                </div>
                ...
                <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-5 border border-white/10">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-blue-400 text-xs font-semibold uppercase tracking-wider">年度关键交付物</span>
                  </div>
                  <ol className="text-slate-300 text-sm space-y-1.5">
                    {(selectedClient?.yearlyDeliverables && selectedClient.yearlyDeliverables.length > 0
                      ? selectedClient.yearlyDeliverables
                      : ['（请在后台填写）']).slice(0, 4).map((t, i) => (
                ...
                <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-5 border border-white/10">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-emerald-400 text-xs font-semibold uppercase tracking-wider">未来14天必做</span>
                  </div>
                  <ul className="text-slate-300 text-sm space-y-1.5">
                    {(selectedClient?.next14Days && selectedClient.next14Days.length > 0
                      ? selectedClient.next14Days
                      : ['（请在后台填写）']).slice(0, 3).map((t, i) => (
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-semibold text-slate-800">季度路线图</h3>
              </div>
              <span className="text-xs text-slate-400 uppercase tracking-wider">Quarterly Roadmap</span>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {([
                { key: 'q1', title: 'Q1', color: 'bg-blue-500', items: selectedClient?.quarterlyPlan?.q1 || [] },
                { key: 'q2', title: 'Q2', color: 'bg-emerald-500', items: selectedClient?.quarterlyPlan?.q2 || [] },
                { key: 'q3', title: 'Q3', color: 'bg-amber-500', items: selectedClient?.quarterlyPlan?.q3 || [] },
                { key: 'q4', title: 'Q4', color: 'bg-rose-500', items: selectedClient?.quarterlyPlan?.q4 || [] },
              ] as const).map((q) => (
                <div key={q.key} className="rounded-xl bg-slate-50 p-4">
                  <div className="flex items-center gap-2 mb-3">
                  ...
                  {q.items.length > 0 ? (
                    <ul className="space-y-1.5 text-xs text-slate-600">
                      {q.items.slice(0, 4).map((t, i) => (
                        <li key={i} className="leading-relaxed">• {t}</li>
                      ))}
                    </ul>
                  ) : (
                    <div className="text-xs text-slate-400">（待填写）</div>
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-xl font-semibold text-slate-800">战略里程碑</h2>
              <p className="text-slate-500 text-sm">Strategic Milestones</p>
            </div>
          </div>
